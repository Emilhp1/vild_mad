...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showProduct(product) {
    ...
    /* Tilføjet af kezia - ignorer den her del for nu */
    /*   const risikoParagraph = document.querySelector(".risiko");

    // Check if the .risiko paragraph contains "Ingen risiko for forveksling"
    if (!risikoParagraph.textContent.trim().length === 0) {
      // If there's no risk, hide the associated icon
      const risikoIcon = risikoParagraph.previousElementSibling;
      if (risikoIcon && risikoIcon.classList.contains(".speech-bubble")) {
        risikoIcon.style.display = "none";
      }
    } */
